...
const sendMail = async ({ recipient, subject, otp }) => {
  ...
  <style>
    ...
    .container { max-width:600px; margin:0 auto; padding:20px; background-color:#09090b; border-radius:40px; text-align:center; }
    h1,p { color:#ffffff; }
  ...
  try {
    const response = await axios.post(
      ...
      {
        sender: { name: "OT-Housing", email: process.env.EMAIL_FROM },
        to: [{ email: recipient }],
        subject,
        htmlContent: html,
      },
